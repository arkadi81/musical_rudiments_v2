function module6ModifierPanel(questionNumber) {
	...
	modifierPanel += '<input type="button" class="hidden-print" value="↑" onClick="module6MoveDegUp(' + questionNumber +')">';
	modifierPanel += '<input type="button" class="hidden-print" value="↓" onClick="module6MoveDegDown(' + questionNumber + ')">';
	modifierPanel += '<input type="button" class="hidden-print" value=&#9839 onClick="module6SharpDeg(' + questionNumber + ')">';
	modifierPanel += '<input type="button" class="hidden-print" value=&#9837 onClick="module6FlatDeg(' + questionNumber + ')">';
	modifierPanel += '<input type="button" class="hidden-print" value="X" onClick="module6DblSharpDeg(' + questionNumber + ')">';
	modifierPanel += '<input type="button" class="hidden-print" Value=&#9837&#9837 onClick="module6DblFlatDeg(' + questionNumber + ')">';
	modifierPanel += '<input type="button" class="hidden-print" Value=&#9838 onClick="module6NaturalizeDeg(' + questionNumber + ')">';	

	return modifierPanel;
}

module6PopulateSelectors = function() {
	...
	var moduleNumber = 6;
  	...
  	for (i=0; i<moduleOptions[moduleNumber].totalQuestions();i++) {
  			...
  			document.getElementById('module' + moduleNumber + 'SelectorPanel'+i).innerHTML+='<table class="table" style="text-align:right">'+module6ModifierPanel(i)+'</table>';
	}
}
...
/*module6PopulateSelectors = function() {
	...
	Deprecated - newer version uses canvas and note modifiers rather than selectors
	...
	for (i=0; i<moduleOptions[moduleNumber].totalQuestions(); i++) {
		//document.getElementById('module' + moduleNumber + 'SelectorPanel' + i).innerHTML += '<select class="form-control" id="module1Selector' + i +'-1">' + rootNoteSelector + '</select>';
...
module6Randomize = function(pullFromServer) {
	...
	if (!pullFromServer) {
		...
		for (i=0; i<moduleOptions[moduleNumber].totalQuestions();i++) { 
			...
			//results.verbalCorrectAnswers[i] = { bottomNote: tmpBottomNote , topNote: tmpTopNote, interval: infoArray[i][2], direction:'above', quality: getFullIntervalName(infoArray[i][2]).quality, size: getFullIntervalName(infoArray[i][2]).size };
			results.verbalCorrectAnswers[i] = { bottomNote: tmpBottomNote , topNote: tmpTopNote, quality: infoArray[i][3], size: infoArray[i][4], direction:'above', root: infoArray[i][2].getRootName(), accidental: accidentalNames[infoArray[i][2].getAccidental()], octave: infoArray[i][2].getOctave() };
...
module6Render = function(renderInfo) {
	...
	for (i=0; i<moduleOptions[moduleNumber].totalQuestions();i++) {
		...
		// display interval
		var intervalText = renderInfo[i][3] + renderInfo[i][4] + ' above' + '<br> \u23AF\u23AF\u23AF\u23AF\u23AF\u23AF\u23AF\u23AF\u23AF\u27F6';
		...
		tmp = new Vex.Flow.StaveNote({ clef:renderInfo[i][0], keys: [bottomNote /*no top note here*/] , duration: "w", color: "black" });
		...
		// DEPRECATED 2/4/2016, color change for answer only
		//module6NaturalizeDeg(i); // just to aling the results
...
module6GetAnswers = function() {
	...
	for (i=0; i<moduleOptions[moduleNumber].totalQuestions(); i++) {
		...
		//var tmpCorrectAcc = moduleResults[moduleNumber].verbalCorrectAnswers[i].accidental;

		var tmpUserDest = moduleResults[moduleNumber].userAnswers[i].value;
		//var tmpUserAcc = moduleResults[moduleNumber].userAnswers[i].accidental.value; 
		...
		/*if (tmpCorrectAcc == tmpUserAcc) {
			moduleResults[moduleNumber].userAnswers[i].accidental.grade=1;
			moduleResults[moduleNumber].totalCorrect +=1;
		}*/
...
// Update: note manipulation buttons 01/31/2016. functions have dedicated module names so that when everything gets dumped things dont mix
// there is no note selection complications - only one note per question to move around

module6MoveDegUp = function(questionNumber) {
	...
	if (moduleResults[moduleNumber].userAnswers[questionNumber].offset<=12) {
		...
		module6RenderResult(questionNumber);
...
module6MoveDegDown =function(questionNumber) {
	
	var moduleNumber = 6;
	
	if (moduleResults[moduleNumber].userAnswers[questionNumber].offset>=-4) {
...
module6FlatDeg =function(questionNumber) {
...
module6SharpDeg =function(questionNumber) {
	var moduleNumber = 6;
...
module6DblFlatDeg =function(questionNumber) {
	var moduleNumber = 6;
...
module6DblSharpDeg =function(questionNumber) {
	var moduleNumber = 6;
...
module6NaturalizeDeg =function(questionNumber) {
	var moduleNumber = 6;
...
module6RenderResult = function (questionNumber) {
	...
	var canvasName = 'module6AnswerCanvas'+questionNumber;
	var newCanvas = document.getElementById(canvasName)
